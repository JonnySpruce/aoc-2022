...
root_size = dirs[Path('/')].size
space_to_free = UPDATE_SIZE - (DISK_SIZE - root_size)

dir_size_to_delete = sorted([size for dir in dirs.values() if (size := dir.size) > space_to_free])[0]
