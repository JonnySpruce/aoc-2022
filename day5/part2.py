...
def extract_instructions(lines: list[str]) -> list[list[int]]:
    ...
    return [[int(line[0]), int(line[1]) - 1, int(line[2]) - 1] for line in instruction_lines]

def execute_instructions(boxes: list[list[str]], instructions: list[list[int]]) -> list[list[str]]:
    for instruction in instructions:
        removed_boxes = boxes[instruction[1]][-instruction[0]:]
        boxes[instruction[1]] = boxes[instruction[1]][:-instruction[0]]
        boxes[instruction[2]].extend(removed_boxes)
    return boxes
